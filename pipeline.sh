...
for PHASE in "${PHASES[@]}"; do
  ...
  while [ "$phase_round" -lt "$MAX_PHASE_ROUNDS" ] && [ "$phase_done" -eq 0 ]; do
    ...
    set +e
    REVIEW_NOTES_FILE="${REVIEW_NOTES_FILE}" REVIEW_LEDGER="${REVIEW_LEDGER:-/tmp/review_ledger.txt}" CRITIC_MODEL="${CRITIC_MODEL:-mimo-v2.5-pro}" bash "$PROJ_ROOT/tests/review_gate.sh" /tmp/phase.diff 2>&1 | tee -a "${RUNLOG}"
    C_EC=${PIPESTATUS[0]}
    set -e
