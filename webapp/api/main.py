...
def decide(tid: int, body: DecideIn) -> dict:
    ...
    if state == "awaiting_design_approval":
        runner.design_decided(body.decision, body.note)
...
async def ws_events(ws: WebSocket, sid: int):
    """Push Event stream for a session. v1: replays recent task events."""
    await ws.accept()
    session = db.get_session(sid)
    if not session:
        ...
        return
    try:
        # Replay the session's task events (latest task).
        tasks = db.list_tasks_by_session(sid)
        for t in tasks[-1:]:
            for ev in db.list_events(t["id"]):
                await ws.send_json(ev)
        # v1: no live push yet — the REST calls return after phases run;
        # the frontend refetches. (Live push comes with the event bus.)
        while True:
