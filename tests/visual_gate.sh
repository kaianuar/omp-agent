...
VISION_MODEL="${GOOGLE_VISION_MODEL:-gemini-3.7-flash}"      # vision model (CommandCode, multimodal)
...
if(!key){ const p=os.homedir()+'/.hermes/.env'; if(fs.existsSync(p)){
  const l=fs.readFileSync(p,'utf8').split('\n').find(l=>l.startsWith('COMMANDCODE_API_KEY='));
...
(async()=>{
  ...
  try{
    const r=await fetch('https://api.commandcode.ai/provider/v1/chat/completions',{method:'POST',
